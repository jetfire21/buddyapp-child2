...
function alex_include_css_js(){
	...
	$user_id_islogin = $user->ID; // get user_id for non-logged user
	global $bp;
...
// for schema.org on google (there was 1 error - missing url breadcrumb) unhoock old function
...
function a_redirect_if_changed_group_page() {
	...
	$uri = $_SERVER['REQUEST_URI'];  // return /i-am/admin7/groups/
...
function as21_get_all_experience_from_page_edit_profile(){
	...
	$fields = $wpdb->get_results( $wpdb->prepare(
		"SELECT ID,post_title,menu_order,guid,comment_count,post_parent,post_password
		FROM {$wpdb->posts}
		WHERE post_author = %d
		    AND post_type = %s
		ORDER BY ID",
