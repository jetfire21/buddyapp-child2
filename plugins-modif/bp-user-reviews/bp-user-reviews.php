...
if ( ! class_exists('BP_Member_Reviews') ) :
    ...
    class BP_Member_Reviews
    {
        ...
        public function ajax_review(){
            ...
            if(!is_array($_POST['criteria'])){
                ...
                if($val < 1 || $val > $stars){
                    $response['result'] = false;
                    $response['errors']['empty'] = __('222222222v You must select all stars', 'bp-user-reviews');
                }
            ...
            if($this->settings['review'] == 'yes') {
                if (empty($_POST['review'])) {
                    ...
                    $response['result'] = false;
                    // $response['errors'][] = sprintf(__('1111111 Review must be at least %s characters', 'bp-user-reviews'), $this->settings['min_length']);
                    $response['errors'][] = sprintf('1111111 Review must be at least %s characters', $this->settings['min_length']);
                } else {
        ...
        public function add_tabs() {
            ...
            $title = sprintf( _x( 'Reviews 111<span class="%s">%s</span>', 'Reviews list sub nav', 'bp-user-reviews' ), esc_attr( $class ), bp_core_number_format( $total ) );
        ...
        public function screen() {
            add_action( 'bp_template_content', array($this, 'screen_content') ,10);
    ...
    // add_action('plugins_loaded', function(){
        // if( class_exists('BuddyPress') ) {
            ...
            remove_action( 'bp_template_content',array($BP_Member_Reviews,'screen_content'),10);
